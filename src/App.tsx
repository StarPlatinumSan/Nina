...
function App() {
	...
	const imagesHim = import.meta.glob("/public/ninette/Him/*.{jpg,jpeg,png,webp}", { eager: true, as: "url" });
	...
	const photosHim: Photo[] = useMemo(
		() =>
			Object.values(imagesHim).map((src, index) => ({
				id: String(index + 1),
				src: src as string,
				alt: `Photo ${index + 1}`,
			})),
						...
						<main className="page">
							...
							<section className="content">
								...
								<motion.section className="carouselSection" variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.25 }}>
									...
									<div className="carouselHeader">
										<h2>Moi 🐕</h2>
										<p>Les photos de moi</p>
									</div>

									<div className="carouselFull">
										<PhotoCarousel photos={photosHim} />
									</div>
